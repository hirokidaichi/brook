...
tmpl.param(param);tmpl.registerFunction(func);return tmpl.output();}};var Klass=function _HTMLTemplate(func){this._param={};this._funcs={};merge(this._funcs,GLOBAL_FUNC);this._output=func;};merge(Klass.prototype,{param:function(obj){merge(this._param,obj);},registerFunction:function(name,func){this._funcs[name]=func;},output:function(){return this._output(this._param,this._funcs);}});var _getSourceFromElement=function(element){var children=Array.prototype.slice.call(element.childNodes||[]);var result=[];for(var i=0,l=children.length;i<l;i++){var e=children[i];if(e.nodeType!=Node.COMMENT_NODE){continue;}
result.push(e.data);}
