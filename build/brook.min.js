...
.use('brook promise')
.define(function(ns){var mapper=function(f){return ns.promise(function(next,val){return next(f(val));});};var through=function(f){return ns.promise(function(next,val){f(val);return next(val);});};var filter=function(f){return ns.promise(function(next,val){if(f(val))return next(val);});};var takeBy=function(by){var num=1;var queue=[];return ns.promise(function(next,val){queue.push(val);if(num++%(by)==0){next(queue);queue=[];}});};var scatter=function(){return ns.promise(function(next,val){for(var i=0,l=val.length;i<l;i++){next(val[i]);}});};var wait=function(msec){var msecFunc=(typeof msec=='function')?msec:function(){return msec};return ns.promise(function(next,val){setTimeout(function(){next(val);},msecFunc());});};var waitUntil=function(f){var p=function(next,val){if(f()){return next(val);}
...
.use('brook promise')
.define(function(ns){var Channel=function(){this.queue=[];this.promises=[];};(function(proto){var through=function(k){return k};proto.send=function(func){var func=(func)?func:through;var _self=this;return ns.promise(function(next,val){_self.sendMessage(func(val));next(val);});};proto.sendMessage=function(msg){this.queue.push(msg);while(this.queue.length){var v=this.queue.shift();for(var i=0,l=this.promises.length;i<l;i++){this.promises[i].run(v);}}};proto.observe=function(promise){for(var i=0;i<this.promises.length;i++){if(this.promises[i]===promise){return;}}
this.promises.push(promise);};proto.stopObserving=function(promise){for(var i=0;i<this.promises.length;i++){if(this.promises[i]===promise){this.promises.splice(i,1);i--;}}};})(Channel.prototype);var channel=function(name){if(name){return getNamedChannel(name);}
...
pairs.push([namespace,map[namespace]]);n(pairs);});var applyNamespace=ns.promise(function(n,pair){Namespace.use([pair[0],'*'].join(' ')).apply(function(ns){n([ns,pair[1]]);});});var registerElements=ns.promise(function(n,v){var _ns=v[0];var widgets=v[1];try{if(_ns.registerElement){for(var i=0,l=widgets.length;i<l;i++){_ns.registerElement.apply(null,widgets[i]);}}else if(_ns.registerElements){var elements=[];for(var i=0,l=widgets.length;i<l;i++){elements.push(widgets[i][0]);}
