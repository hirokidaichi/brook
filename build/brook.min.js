...
continue;origin[prop]=target[prop];}};var meta={'\b':'\\b','\t':'\\t','\n':'\\n','\f':'\\f','\r':'\\r','"':'\\"','\\':'\\\\'};var quote=function(str){return'"'+str.split('').map(function(e){return meta[e]?meta[e]:e;}).join('')+'"';};var GLOBAL_FUNC={__escapeHTML:function(str){return str
.toString()
.replace(/&/g,'&amp;')
.replace(/</g,'&lt;')
.replace(/>/g,'&gt;')
...
tmpl.param(param);tmpl.registerFunction(func);return tmpl.output();}};var Klass=function _HTMLTemplate(func){this._param={};this._funcs={};merge(this._funcs,GLOBAL_FUNC);this._output=func;};merge(Klass.prototype,{param:function(obj){merge(this._param,obj);},registerFunction:function(name,func){this._funcs[name]=func;},output:function(){return this._output(this._param,this._funcs);}});merge(Klass,{cache:{},get:function(source){var uniqId='autocache:'+Klass.hashFunction(source);var func=Klass.resolve(uniqId);if(func)
return new Klass(func);return new Klass(Klass.reserve(uniqId,source));},resolve:function(name){return Klass.cache[name];},reserve:function(name,source){var functionBody=ns.getFunctionText(source);Klass.cache[name]=ns.compileFunctionText(functionBody);return Klass.cache[name];},getByElementId:function(elementId){var uniqId='dom:'+elementId;var func=Klass.resolve(uniqId);if(func){return new Klass(func);}
...
.join('');return new Klass(Klass.reserve(uniqId,source));},hashFunction:function(string){var max=(1<<31);var length=string.length;var ret=34351;var pos='x';for(var i=0;i<length;i++){var c=string.charCodeAt(i);ret*=37;pos^=c;ret+=c;ret%=max;}
